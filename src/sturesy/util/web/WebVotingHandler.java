...
public class WebVotingHandler implements IPollPlugin {
	...
	public static final LectureID NOLECTUREID = new LectureID(
			"error.no.lecture.id", "this.is.no.password",
			"http://www.no-lecture-id.com/");
	...
	private void gatherVotes() {
		try
		{
			parseVotesJSON(WebCommands.getVotes(_lectureid.getHost(),
					_lectureid.getLectureID(), _lectureid.getPassword()));
	...
	private void updateAnswerTypeWebsite() {
		try
		{
			WebCommands.updateQuestion(_lectureid.getHost().toString(),
					_lectureid.getLectureID(), _lectureid.getPassword(),
					_question);
		} catch (JSONException e)
		{
	...
	private void clearPreviousVotings() {
		try
		{
			WebCommands.cleanVotes(_lectureid.getHost().toString(),
					_lectureid.getLectureID(), _lectureid.getPassword());
		} catch (JSONException e)
		{
			Log.error("Error parsing json data: " + e.getMessage());
	...
	private void parseVotesJSON(String jsonString) {
		...
		if (!jsonString.startsWith("NO DATA") && jsonString.startsWith("{")
				&& jsonString.endsWith("}"))
		{
			try
			{
				JSONObject jsonObject = new JSONObject(jsonString);
				Iterator<?> firstLevelIterator = jsonObject.keys();
				while (firstLevelIterator.hasNext())
				{
					String firstLevelKey = (String) firstLevelIterator.next();
					JSONObject firstlevel = jsonObject
							.getJSONObject(firstLevelKey);
					String guid = firstlevel.getString("0");
					String dateString = firstlevel.getString("2");
					long timediff = getTimeDiff(dateString);

					Object votes = firstlevel.get("1");

					if (votes instanceof JSONObject)
					{
						JSONObject votesarray = (JSONObject) votes;
						ArrayList<Short> voteslist = new ArrayList<Short>();
						Iterator<?> secondLevelIterator = votesarray.keys();
						while (secondLevelIterator.hasNext())
						{
							String secondLevelKey = (String) secondLevelIterator
									.next();
							Object possibleInt = votesarray.get(secondLevelKey);
							if (possibleInt instanceof Integer)
							{
								voteslist.add(((Integer) possibleInt)
										.shortValue());
							}
						}
						resultList.add(new MultipleVote(guid, timediff,
								voteslist));
					} else if (votes instanceof Integer)
					{
						resultList.add(new SingleVote(guid, timediff,
								(Integer) votes));
					} else if (votes instanceof String)
					{
						resultList.add(new TextVote(guid, timediff,
								(String) votes));
					}
				}
			} catch (JSONException e)
